...
- **2026-05-30 (latest)** — Phase 6: account-number accuracy fix (old/new BBANK2 resolution) in AI conversation replies:
  - **Root cause**: A rep asked how account **51149** was tracking; the AI reply came back ~$19k when Carpet Residential alone is ~$99k (and the full multi-category total is higher). Reps universally refer to accounts by their **OLD (BBANK2)** number (e.g. 51149), but the warehouse keys all sales on the **NEW** account number (`BACCT#`/`BSACCT`, which can be alphanumeric like `PROHAR`). `_AiReplyWorker._fetch_account_detail` filtered `df["account_number"].isin(["51149"])` against the NEW column only — so the referenced account never matched. The $19k the AI cited was scraped from **truncated** per-product/per-account slices in the rep dashboard (`_fetch_rep_data` caps top-N products × top-N accounts), which structurally undercount. The AI even flagged it: "The account number in the source data is #PROHAR, not #51149."
  - **Fix 1 — new pure resolver `_AiReplyWorker._resolve_referenced_accounts(asgn, account_numbers, restrict_to_accounts=None)`** (`app/ui/views/conversations_view.py`): matches each user-typed token (with `#` stripped) against BOTH `account_number` (new) AND `old_account_number` (BBANK2) from `load_rep_assignments`, returning `{new_account: {name, old, closed}}`. `restrict_to_accounts` (rep mode) limits resolution to the rep's own accounts so a rep can never pull another rep's account by guessing a shared/old number. Extracted as a `@staticmethod` so it is unit-testable without Qt/DB.
  - **Fix 2 — `_fetch_account_detail` rewritten** to a COMPLETE, AUTHORITATIVE breakdown: resolves old/new → canonical NEW account(s), loads full invoiced sales (`code_prefix="0"`, product CCs) for FY YTD + prior FY YTD, then emits per account: a **TOTAL across ALL categories** with YoY + GP%, a **BY CATEGORY (cost center)** table (complete, no truncation, with CC names via `load_all_cost_centers`), a **BY PRODUCT** list (price-class descriptions via `load_price_class_lookup`), and a **BY MONTH** breakdown. Account labels show BOTH numbers (`KMAF ASSOCIATES (#51149)  [new acct PROHAR]`) and a `[CLOSED]` suffix when applicable. When the number is unrecognized or has no sales, emits an explicit NOT-FOUND / NO-SALES note so the AI never substitutes or guesses. Rep call site passes `restrict_to_accounts=self._rep_accounts`.
  - **Fix 3 — both AI system messages gained an `ACCOUNT TOTALS — CRITICAL ACCURACY RULE`** (mgmt + rep branches): the `REFERENCED ACCOUNT — COMPLETE & AUTHORITATIVE BREAKDOWN` section is the ONLY source of truth for an account total; the AI must NEVER total an account by summing truncated per-product/per-rep slices; old vs new numbers are the SAME account (echo the number the reader used); NOT-FOUND/no-sales must be stated plainly.
  - **Tests**: new `tests/test_account_resolution.py` (5 cases: old→new resolution, direct new match, `#`-strip + closed-flag, rep-scope restriction blocking another rep's account on a shared old number, unknown number → empty). **53/53 pass.**
  - **Recommendation (logged for the user)**: long-term, a single shared old↔new account resolver should back EVERY user-facing account lookup (Ask AI view, weekly email), and every AI surface should echo both the typed and resolved canonical numbers so a mismatch can never be hidden.

- **2026-05-29 (previous latest)** — Phase 5: monthly cadence (90-day anti-repetition), rep reply privacy scoping + company benchmark averages, reply-guidance footer, rep-preference custom email section:
  - **90-day anti-repetition (monthly cadence)** (`app/ui/views/weekly_email_view.py`): `_recent_emails_for` now calls `recent_weekly_email_texts(salesman_number, days=90)` (was 30) — cadence is monthly, so the lookback spans ~3 prior emails. `_recent_emails_block` header text and the `_build_rep_prompt` FRESHNESS rule both updated "last ~30 days" → "last ~90 days".
  - **Rep reply privacy scoping (security-critical)** (`app/ui/views/conversations_view.py`): New `_AiReplyWorker._fetch_benchmark_averages()` — loads company-wide invoiced sales (cached layer), applies the active-rep SALESMAN whitelist, and emits a `COMPANY BENCHMARK AVERAGES` block containing ONLY aggregates/averages (active reps, avg+median revenue/rep, avg accounts/rep, avg revenue/account, company GP%, and avg-per-rep by top-10 product line). NEVER names or breaks out any individual rep. Wired into the rep branch of `_generate` (appended to `warehouse_block`). The rep system message gained a `DATA SCOPE & PRIVACY — CRITICAL` section: rep may only receive SPECIFIC figures for their OWN territory; MUST politely decline any request for another individual rep's specific numbers (and offer the benchmark instead); MAY answer company-wide AVERAGE/BENCHMARK questions from the benchmark block only. Defense-in-depth: the rep branch physically loads only the rep's own territory detail, so other reps' specifics are not present in context — the prompt rule is reinforcement, not the sole guard. The old "by rep for all reps → say you only have your territory" line was reworked to point at the benchmark block.
  - **Reply-guidance footer** (`weekly_email_view.py`): New `_reply_guidance_html()` appended inside `_wrap_ai_body` (per-rep emails only; master leaderboard uses a separate renderer and is unaffected). Premium light-blue card inviting the rep to reply, with examples of good question types (account trend, product line in territory, open-order pipeline, comparison to company average, and "tell me what you'd like to see in future emails"). Explicitly states it can share the rep's own detail + company averages but not another rep's individual numbers.
  - **Rep-preference custom section + clear delineation** (`weekly_email_view.py` + `conversations_view.py`): `_extract_and_save_facts` already persists `fact_type='preference'` from rep replies; these surface via `rep_facts_block`. The `_build_rep_prompt` sys_msg gained a `REP REQUESTS / CUSTOM CONTENT — CLEAR DELINEATION` rule: standard 3 sections come first, then any rep-requested content goes under a separate, clearly delineated `——— YOU ASKED TO SEE ———` header (omitted entirely when no relevant preference exists; data-only, never fabricated). The rep reply sys_msg gained a `FUTURE-EMAIL PREFERENCES` rule instructing the AI to acknowledge in one sentence that a stated preference is noted and will be applied going forward (saved automatically).
  - **Visuals**: per-rep trend chart (`_generate_trend_chart_html`) confirmed wired into `_wrap_ai_body` via `chart_html`; no over-engineering added (user said charts optional).
  - **Tests**: new `tests/test_weekly_email_helpers.py` (3 cases: reply-guidance content, 90-day window text, excerpt truncation). **48/48 pass.**

- **2026-05-28 (previous latest)** — Phase 4: second-person weekly emails, durable rep-fact memory, conversations feedback loop, completeness footnotes, relevance validation:
  - **Second-person voice + shorter/balanced weekly emails** (`app/ui/views/weekly_email_view.py`): `_build_rep_prompt` sys_msg rewritten — "You are a senior sales manager writing a short, personal weekly email DIRECTLY TO one of your sales reps." VOICE—CRITICAL mandates SECOND PERSON ("you"/"your"), forbids rep-name-possessive. LENGTH 110–180 words. Three sections: "WHAT'S GOING WELL (2 bullets)", "WHAT'S TRENDING THE WRONG WAY (2–3 bullets)", "WHAT TO FOCUS ON THIS WEEK (1 action)". `_fallback_body` rewritten to match second-person 3-section structure. Tier logic, tone ladder, category-integrity, closed-account, marketing-program rules retained.
  - **Anti-repetition (30-day window)**: `recent_weekly_email_texts(rep_id, days=30)` in `repos.py` pulls up to 6 recent outbound weekly-email bodies; `_recent_emails_block(...)` injects them into the prompt with a "do NOT reuse the same insights" instruction. Wired via `WeeklyEmailView._recent_emails_for(slmn)`.
  - **Durable rep-fact memory (auditable + reversible)**: new `rep_facts` table (`app/storage/schema.py`, `CURRENT_SCHEMA_VERSION = 3`) — `rep_id, account_number, account_label, fact_type (account_closed|account_note|preference|other), fact_text, source (rep_feedback|manager), source_message_id, conversation_id, active, created_at, updated_at` + indexes. `repos.py` adds `RepFact` dataclass, `save_rep_fact` (dedups on rep+account+type), `list_rep_facts`, `set_rep_fact_active`, `delete_rep_fact`, `closed_account_numbers`, and `rep_facts_block(rep_id)` (prompt block honoring/flagging closed accounts).
  - **Conversations feedback → memory loop** (`app/ui/views/conversations_view.py`): `_AiReplyWorker._extract_and_save_facts(rep_id, last_body, inbound)` — small temp-0 AI call returns a JSON array of durable facts the rep asserted (e.g. "that account is closed") and persists each via `save_rep_fact(source='rep_feedback', ...)`. Runs before reply generation (fail-open, manager senders skipped). `rep_facts_block(rep_id)` is injected into the warehouse block for both rep + management branches so replies acknowledge and honor known facts. Both sys_msgs gained a "KNOWN FACTS / DATA COMPLETENESS — CRITICAL" section.
  - **Completeness footnotes**: `_generate` tracks `completeness_notes` when a `_fetch_*` returns empty unexpectedly; injects a DATA COMPLETENESS WARNING into the user_msg and instructs the AI to end the reply with a `⚠ Note:` line stating what may be missing. Both sys_msgs reinforce the rule.
  - **Validator relevance check**: `_validate_draft(draft, warehouse_block, rep_question="")` extended — new rules 7 (RELEVANCE: the draft must actually answer the reader's request) and 8 (completeness `⚠ Note:` enforcement). `_generate` now passes `rep_question=last_body`.
  - **Rep Facts audit UI**: new "Rep Facts" tab on `ConversationsView` — `QTableWidget` (Rep, Account, Fact, Type, Source, When) via `list_rep_facts(active_only=...)`, "Show inactive" toggle, and Deactivate/Reactivate + Delete buttons (`set_rep_fact_active` / `delete_rep_fact`). Refreshes with the view.
  - **Tests**: new `tests/test_rep_facts.py` (3 cases: save+dedup, deactivate+delete, closed-account set + block). **45/45 pass.**

- **2026-05-27 (previous latest)** — OpenAI 429 retry + per-category account list in conversations mgmt & weekly master:
  - **Root cause of 429**: `OpenAIProvider.complete()` had zero retry on HTTP 429 (rate limit) or 5xx — any short OpenAI rate-limit window propagated straight up as `HTTPStatusError` and broke the user's flow. The previous CCA fix also enlarged the Ask AI prompt (per-category × per-account block) which moved more requests close to the TPM ceiling.
  - **Fix 1 — retry-with-backoff in `app/ai/openai_provider.py`**: New constants `_RETRY_STATUS_CODES = {429, 500, 502, 503, 504}`, `_MAX_RETRIES = 4`, `_BASE_BACKOFF_SECONDS = 2.0`. `complete()` now loops up to 5 attempts; on a retriable status it honors `Retry-After` (seconds) when present, otherwise uses exponential backoff `2s, 4s, 8s, 16s` plus 0–1s jitter, capped at 60s per wait. Transport-level transients (`ConnectError`, `ReadTimeout`, `RemoteProtocolError`) also retry. Final attempt either returns success or raises the original error so non-retriable failures still surface immediately.
  - **Fix 2 — Ask AI prompt size**: `_MAX_PER_CAT = 50` cap on the new per-category × per-account listing; overflow shows `… and N more (filter on the CSV column 'marketing_categories' for the full list)`. Full data is still in the CSV columns so accuracy isn't lost — only the duplicated authoritative listing is bounded.
  - **Fix 3 — same hallucination class in mgmt/master AI surfaces**: The previous CCA fix only touched Ask AI. Audit revealed `_AiReplyWorker._fetch_marketing_programs_block` (conversations management mode) and `WeeklyEmailView._marketing_programs_block(None)` (master leaderboard) only injected aggregate counts ("CCA Buying Group: 73 enrolled accounts") with no per-account listing. A management user asking "which CCA customers grew last quarter" or the master BI prompt referencing a category would hit the exact same guessing failure mode.
  - **New shared helper `category_to_accounts_lines(...)` in `app/services/marketing_programs.py`**: emits `[Category] — N enrolled account(s):` blocks listing each account (with optional label) under its category heading, skipping `UNCATEGORIZED`. Honors `account_filter` (rep scope) and `account_labels` (display names). Caps at `max_per_category=80` (default) with truncation note.
  - **Wired into both surfaces**:
    - `conversations_view._fetch_marketing_programs_block` appends the listing for both rep and management modes; rep mode passes `account_labels=self._rep_account_labels` and `account_filter=<rep accts>`, mgmt mode passes both as None for full-org scope.
    - `weekly_email_view._marketing_programs_block` builds `label_map` from `self._assignments_df` (scoped to `rep_accounts` when non-None) and appends the listing. Master mode (`rep_accounts=None`) gets full-org listing; per-rep mode is scoped.
  - **Tests**: 2 new in `tests/test_marketing_programs.py` — `test_category_to_accounts_lines_lists_membership_and_skips_uncategorized` and `test_category_to_accounts_lines_respects_account_filter_and_truncates`. **42/42 pass.**

- **2026-05-27 (previous latest)** — Ask AI: per-account marketing-category filter (fixes "CCA accounts" returning non-CCA accounts):
