...
- **2026-05-18 (latest)** — Auto-reply uses real warehouse data; hallucination blocked:
  ...
  - **System prompt hardened** with CRITICAL RULES: "NEVER invent, estimate, or fabricate any number. If a specific figure is not in FRESH WAREHOUSE DATA, say exactly: 'I don't have that breakdown in the warehouse right now'." Temperature lowered from 0.35 → 0.2 for factual consistency. Product names and account labels must be used verbatim from the data. Exact date range must be cited in every response.
  - **30/30 tests pass.**
