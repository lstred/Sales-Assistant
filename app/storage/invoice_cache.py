...
_TABLE_DDL = """
CREATE TABLE IF NOT EXISTS invoice_month_cache_v4 (
...
def _get_cached(year: int, month: int, code_prefix: str) -> pd.DataFrame | None:
    with closing(_connect()) as conn:
        row = conn.execute(
            "SELECT payload FROM invoice_month_cache_v4 "
...
def _put_cached(year: int, month: int, code_prefix: str, df: pd.DataFrame) -> None:
    ...
    with closing(_connect()) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO invoice_month_cache_v4 "
...
def has_any() -> bool:
    with closing(_connect()) as conn:
        row = conn.execute("SELECT COUNT(*) FROM invoice_month_cache_v4").fetchone()
    return bool(row and row[0])


def clear_all() -> int:
    with closing(_connect()) as conn:
        cur = conn.execute("DELETE FROM invoice_month_cache_v4")
        conn.commit()
...
def stats() -> tuple[int, int]:
    ...
    with closing(_connect()) as conn:
        row = conn.execute(
            "SELECT COUNT(*), COALESCE(SUM(rows), 0) FROM invoice_month_cache_v4"
        ).fetchone()
