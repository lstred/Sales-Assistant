...
class WeeklyEmailView(QWidget):
    ...
    def _generate_master(self) -> None:
        ...
        cur_start, _cur_end = current_week_range(today)
        cur_to_date_end = today

        per_rep_last = revenue_in_window(self._df, wk_start, wk_end, by="rep")
        per_rep_curr = revenue_in_window(self._df, cur_start, cur_to_date_end, by="rep")
        ...
        body_html = _render_master_html(
            ...
            cur_start=cur_start, cur_end=cur_to_date_end,
    ...
    def _ai_shoutouts(self, leaderboard: list[tuple[str, float]]) -> dict[str, str]:
        ...
        for rep, rev in leaderboard:
            ...
            yoy = "n/a" if sc is None or sc.yoy_pct is None else f"{sc.yoy_pct:+.1f}%"
            top = (
                sc.top_growing_accounts[0]["account"]
                if sc and sc.top_growing_accounts else "n/a"
            )
            bullets.append(
                f"- {rep}: week ${rev:,.0f}; rolling YoY {yoy}; top growing account {top}"
            )
    ...
    def _weekly_lines_for(self, rep_key: str) -> dict:
        ...
        cur_s, _cur_e = current_week_range(today)
        df = df.copy()
        df["invoice_date"] = pd.to_datetime(df["invoice_date"], errors="coerce")
        prev_mask = (df["invoice_date"] >= pd.Timestamp(prev_s)) & (df["invoice_date"] <= pd.Timestamp(prev_e))
        cur_mask = (df["invoice_date"] >= pd.Timestamp(cur_s)) & (df["invoice_date"] <= pd.Timestamp(today))
        return {
...
def _scorecard_footer_html(sc: RepScorecard) -> str:
    ...
    growing = ", ".join(a["account"] for a in sc.top_growing_accounts[:3]) or "\u2014"
    declining = ", ".join(a["account"] for a in sc.top_declining_accounts[:3]) or "\u2014"
    return (
        f"<hr style='border:none;border-top:1px solid #E2E8F0;margin:18px 0;'>"
        f"<div style='font-size:12px;color:#334155;'>"
        f"<p style='margin:6px 0;'><b>Scorecard</b></p>"
    ...
    *,
    ...
    for i, (rep, rev) in enumerate(sorted_last, 1):
        ...
        rows.append(
            ...
            f"<td style='padding:6px 8px;'><b>{rep}</b></td>"
            f"<td style='padding:6px 8px;text-align:right;'>${rev:,.0f}</td>"
            f"<td style='padding:6px 8px;text-align:right;'>${cur:,.0f}</td>"
            f"<td style='padding:6px 8px;color:#475569;'>{msg}</td>"
            f"</tr>"
        )
    table_rows = "".join(rows) or (
        "<tr><td colspan='5' style='padding:8px;color:#888;'>No invoiced sales last week.</td></tr>"
    )
    overview = ""
    if period_overview is not None and period_overview.revenue:
        overview = (
            f"<p><b>{period_overview.label} \u2014 company recap:</b> "
            f"${period_overview.revenue:,.0f}"
            + ("" if period_overview.yoy_pct is None
               else f" ({period_overview.yoy_pct:+.1f}% YoY)")
    ...
    return (
        "<p>Team \u2014 here\u2019s the scoreboard for last week.</p>"
        + overview
        + f"<h3>Week of {week_start.isoformat()} \u2192 {week_end.isoformat()}</h3>"
        + "<table cellpadding='0' cellspacing='0' "
          "style='border-collapse:collapse;width:100%;font-size:13px;'>"
        + "<thead><tr style='background:#F1F5F9;'>"
          "<th style='padding:6px 8px;text-align:left;'>#</th>"
          "<th style='padding:6px 8px;text-align:left;'>Rep</th>"
          "<th style='padding:6px 8px;text-align:right;'>Last week</th>"
          "<th style='padding:6px 8px;text-align:right;'>Week to date</th>"
          "<th style='padding:6px 8px;text-align:left;'>Shout-out</th>"
          "</tr></thead>"
...
def _build_rep_prompt(
    *,
    ...
    growing_lines = "\n".join(
        f"  - {a['account']}: ${a['current']:,.0f} (was ${a['prior']:,.0f}, {a['delta']:+,.0f})"
    ...
    stale_lines = "\n".join(
        f"  - {a['account']}: was ${a['prior']:,.0f} prior period, $0 this period"
        for a in sc.stale_accounts
    ) or "  (none)"
    new_lines = "\n".join(
        f"  - {a['account']}: ${a['current']:,.0f} this period (was $0)"
...
def _fallback_body(
    ...
    if sc.top_growing_accounts:
        ...
        parts.append(
            f"Nice work on {a['account']} \u2014 you're up "
            f"${a['delta']:,.0f} on that account vs last year."
        )
    yoy = sc.yoy_pct
    if yoy is not None and sc.peer_avg_yoy_pct is not None:
        parts.append(
    ...
    if sc.top_declining_accounts:
        ...
        parts.append(
            f"Biggest opportunity: {a['account']} is off "
            f"${-a['delta']:,.0f} \u2014 worth a focused conversation."
        )
    if sc.stale_accounts:
        a = sc.stale_accounts[0]
        parts.append(
            f"Heads-up: {a['account']} bought ${a['prior']:,.0f} last year "
            "and nothing this period. A visit or call could re-engage them."
        )
    parts.append(
...
def _fallback_shoutout(rep_key: str, sc: RepScorecard | None) -> str:
    ...
    if sc.top_growing_accounts:
        a = sc.top_growing_accounts[0]
        return f"Big win at {a['account']} (+${a['delta']:,.0f} YoY)."
    if sc.new_accounts:
        a = sc.new_accounts[0]
        return f"Opened up {a['account']} this period \u2014 great prospecting."
    if sc.yoy_pct is not None and sc.yoy_pct > 0:
